...
one quote and returns validated quote data plus a sanitized continuation
descriptor; it never authenticates a wallet, opens a session, prepares an
unsigned action, signs, or submits. It fails closed on anything that claims the
server will sign or submit.
...
class AssetFareQuoteTool(Tool):
    ...
    description = (
        ...
        "the returned 'server_signs_or_submits' is always false. It returns "
        "expected/minimum receive amounts (native and USD), the "
        ...
        "upstream handoff safety assertion is missing or malformed, or "
        ...
        "swap, sign or move funds. It strictly validates the full continuation_v3 and returns only "
        "a sanitized continuation_descriptor with non-executable discovery metadata. The tool never creates "
        "approval_v3 or selects a mode; the caller must explicitly choose exactly one "
        "allowed mode. Multi-step routes are session-only. caller_approved alone is "
        "not proof of human approval and legacy handoff is advisory."
    )
    inputs = {
        "from_chain": {
            "type": "string",
