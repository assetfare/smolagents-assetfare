"""AssetFare caller-approved one-shot /v2/prepare tool for smolagents agents.

Self-contained smolagents ``Tool`` (Hub-loadable via ``load_tool``). It performs the
explicit, caller-approved, non-custodial one-shot ``POST /v2/prepare`` and returns a
validated, bounded FIRST unsigned action bundle for a fresh re-quote. It NEVER signs,
NEVER submits, NEVER receives a private key or seed, and is NEVER auto-called from a
quote: the caller must invoke it deliberately with ``caller_approved`` set to the
literal ``True`` and the route's own PUBLIC wallet addresses.
...
Fail-closed guarantees:
  ...
  - the returned bundle must explicitly assert it is neither signed nor submitted and
    that the server neither signs nor submits, else it fails closed.
...
Injectable ``session`` / ``monotonic`` let the tool be unit-tested offline with mocks.
"""
...
class AssetFarePrepareTool(Tool):
    ...
    inputs = {
        "caller_approved": {
            "type": "boolean",
            "description": "Must be the literal boolean true. It is the explicit caller approval gate; anything else is rejected before any network call.",
        },
        "from_chain": {
            "type": "string",
